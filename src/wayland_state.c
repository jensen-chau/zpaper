...
struct wayland_state *wayland_state_new() {
  struct wayland_state *state =
      (struct wayland_state *)malloc(sizeof(struct wayland_state));
...
int create_layer_surface(struct wayland_state *state) {
  for (int i = 0; i < state->output_count; i++) {
    ...
    info->viewport = wp_viewporter_get_viewport(state->viewporter, info->surface);
    ...
    zwlr_layer_surface_v1_add_listener(info->layer_surface, &zwlr_layer_surface_v1_listener, state);
